...
import { ArrowUp, ChevronDown, Menu, Moon, Search, Sun, X } from 'lucide-react';
      ...
      <header className={`sticky top-0 z-50 border-b backdrop-blur-xl ${isDark ? 'border-white/10 bg-midnight/80' : 'border-slate-200 bg-white/80'}`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden flex-1 items-center justify-center gap-3 lg:flex">
            ...
            <button type="button" className={`rounded-full border p-2 ${isDark ? 'border-white/10 text-slate-200' : 'border-slate-200 text-slate-700'}`} onClick={() => setTheme((prev) => (prev === 'dark' ? 'light' : 'dark'))} aria-label="Toggle theme">
          ...
          <div className="flex items-center gap-2 lg:hidden">
            ...
            <button type="button" className={`rounded-full border p-2 ${isDark ? 'border-white/10 text-slate-200' : 'border-slate-200 text-slate-700'}`} onClick={() => setMobileNavOpen((prev) => !prev)} aria-label="Toggle navigation">
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className={`overflow-hidden border-t lg:hidden ${isDark ? 'border-white/10 bg-[#060b1b]' : 'border-slate-200 bg-white'}`}>
              <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6">
                <div className="space-y-2">
                  ...
                  <Link to="/advertise" onClick={() => setMobileNavOpen(false)} className={`block rounded-[1rem] border px-4 py-3 text-sm font-medium ${isDark ? 'border-white/10 bg-white/5 text-slate-100' : 'border-slate-200 bg-slate-50 text-slate-800'}`}>Advertise</Link>
                </div>
              </div>
            </motion.div>
          )}
